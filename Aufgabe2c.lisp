...
;; Generiere alle möglichen Ort-ID-Paare
;; Filtern Sie die Paare heraus, die einen Übergang haben
;; Ausgabe der Ort-ID-Paare ohne Übergang
;;Geben Sie jedes unsortierte Ort-ID-Paar aus, von welchem es
;;keinen Übergang vom ersten zum
;;zweiten gibt.


(as Ort1
    (rename-attribute OrtID1 OrtID Ort))

(as Ort2
    (rename-attribute OrtID2 OrtID Ort))

(as alle-unsortierten-ort-paare
    (project (OrtID1 Adresse1 OrtID2 Adresse2)
             (select (or (= Adresse1 "Luftnation") (= Adresse1 "Wassernation") (= Adresse1 "Erdnation") (= Adresse1 "Feuernation"))
                     (x (rename-attribute Adresse1 Adresse Ort1) (rename-attribute Adresse2 Adresse Ort2)))))
 

(as keinen-uebergang-zum-ersten
    (project (OrtID1 Adresse1 OrtID2 Adresse2)
             (select (and (= Von OrtID1) (/= Nach OrtID2))
                     (x alle-unsortierten-ort-paare Uebergang))))

(format t "~% Alle Orte ohne Uebergang vom ERSTEN zum ZWEITEN:~% ~a" (rows keinen-uebergang-zum-ersten))
